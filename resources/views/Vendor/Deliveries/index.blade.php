                            ...
                            <div class="table-responsive">
                                ...
                                <table class="table table-de mb-0 " id="seancetab" border="2" cellpadding="3">
                                    ...
                                    <tbody>
                                      @forelse ($items as $i )
                                      <tr>
                                            <th>{{ $i->product->designation }}</th>
                                            <th>{{ $i->provider->nom." ".$i->provider->prenom }}</th>
                                            <th>{{ $i->date_livraison }}</th>
                                            <th>{{ $i->qte }}</th>
                                            <th>
                                                <div class="btn-group" role="group" aria-label="Basic Example">
                                                    <a href="{{ route('stock.DeliveryEdit', $i->id) }}"
                                                        class="btn btn-info btn-min-width box-shadow-3 mr-1 mb-1">
                                                        <i class="la la-edit"></i> Modifier
                                                    </a>
                                                    <a href="{{ route('stock.DeliveryDelete', $i->id) }}"
